...
public class HotelTables {
	...
	private static void createTables(){
		try{
			...
			statement.executeUpdate("CREATE TABLE manage (managerID INT,representativeID INT,PRIMARY KEY(managerID,representativeID),CONSTRAINT manage_ma_fk FOREIGN KEY(managerID) REFERENCES manager(staffID) ON UPDATE
															CASCADE,CONSTRAINT manage_re_fk FOREIGN KEY(representativeID) REFERENCES representative(staffID) ON UPDATE CASCADE;")

		  statement.executeUpdate("CREATE TABLE register (representativeID INT,customerID INT,PRIMARY KEY(representativeID,customerID),CONSTRAINT register_re_fk FOREIGN KEY(representativeID) REFERENCES representative(staffID)
																ON UPDATE CASCADE,CONSTRAINT register_cus_fk FOREIGN KEY(customerID) REFERENCES customerInfo(customerID)ON UPDATE CASCADE);")

			statement.executeUpdate("CREATE TABLE manageHotel (managerID INT,hotelID INT,PRIMARY KEY(managerID, hotelID),CONSTRAINT manageHotel_manager_fk FOREIGN KEY(managerID) REFERENCES manager(staffID)
																ON UPDATE CASCADE,CONSTRAINT manageHotel_hotel_fk FOREIGN KEY(hotelID) REFERENCES hotel(hotelID) ON UPDATE CASCADE);")

			statement.executeUpdate("CREATE TABLE checkinInfo (checkinID INT PRIMARY KEY AUTO_INCREMENT,hotelId INT NOT NULL,CONSTRAINT checkinInfo_hotel_fk FOREIGN KEY(hotelID) REFERENCES hotel(hotelID)
																ON UPDATE CASCADE,customerName VARCHAR(128) NOT NULL, roomID INT NOT NULL, guestsNumber VARCHAR(128) NOT NULL, customerBirthday DATE, startDate DATE NOT NULL,
																endDate DATE NOT NULL, checkinTime DATE NOT NULL, checkoutTime DATE NOT NULL, serviceOffered VARCHAR(128) );")

			statement.executeUpdate("CREATE TABLE assign (checkinID INT,representativeID INT NOT NULL,roomID INT NOT NULL,customerID INT NOT NULL,PRIMARY KEY(checkinID),CONSTRAINT assign_checkin_fk FOREIGN KEY(checkinID) REFERENCES checkinInfo(checkinID)
																	ON UPDATE CASCADE,CONSTRAINT assign_representative_fk FOREIGN KEY(representativeID) REFERENCES representative(staffID) ON UPDATE CASCADE,CONSTRAINT assign_room_fk FOREIGN KEY(roomID) REFERENCES room(roomID)
																	ON UPDATE CASCADE,CONSTRAINT assign_customer_fk FOREIGN KEY(customerID) REFERENCES customerInfo(customerID) ON UPDATE CASCADE);")

			statement.executeUpdate("CREATE TABLE serviceOffered (serviceOfferedID INT PRIMARY KEY AUTO_INCREMENT,checkinID INT NOT NULL,staffID INT NOT NULL,price FLOAT NOT NULL,CONSTRAINT serviceOffered_checkin_fk FOREIGN KEY(checkinID) REFERENCES checkinInfo(checkinID)
																	ON UPDATE CASCADE,CONSTRAINT serviceOffered_staff_fk FOREIGN KEY(staffID) REFERENCES staff(staffID) ON UPDATE CASCADE);")
